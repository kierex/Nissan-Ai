...
const watchAndReloadConfig = (dir, type, prop, logName) => {
        ...
        fs.watch(dir, (eventType) => {
                if (eventType === type) {
                        const oldConfig = global.CowBot[prop];
                        ...
                        setTimeout(() => {
                                try {
                                        ...
                                        global.CowBot[prop] = JSON.parse(fs.readFileSync(dir, 'utf-8'));
                                        log.success(logName, `Reloaded ${dir.replace(process.cwd(), "")}`);
                                }
                                catch (err) {
                                        log.warn(logName, `Can't reload ${dir.replace(process.cwd(), "")}`);
                                        global.CowBot[prop] = oldConfig;
                                }
                                finally {
...
global.CowBot.envGlobal = global.CowBot.configCommands.envGlobal;
global.CowBot.envCommands = global.CowBot.configCommands.envCommands;
global.CowBot.envEvents = global.CowBot.configCommands.envEvents;
...
class MemoryManager {
	...
	_performCleanup() {
		...
		if (CowBot.oldListening.length > this.options.maxOldListening) {
			const toRemove = CowBot.oldListening.length - this.options.maxOldListening;
			for (let i = 0; i < toRemove; i++) {
				const handle = CowBot.oldListening.shift();
		...
		const callbackEntries = Object.keys(CowBot.callbackListenTime);
		if (callbackEntries.length > this.options.maxCallbackListenTime) {
			// Sort by timestamp and remove oldest
			const sorted = callbackEntries
				.map(key => ({ key, time: CowBot.callbackListenTime[key] }))
				.sort((a, b) => a.time - b.time);

			const toRemove = sorted.length - this.options.maxCallbackListenTime;
			for (let i = 0; i < toRemove; i++) {
				delete CowBot.callbackListenTime[sorted[i].key];
			}
			cleaned += toRemove;
		}

		// Cleanup onFirstChat if too large
		if (CowBot.onFirstChat.size > this.options.maxOnFirstChatSize) {
			const entries = Array.from(CowBot.onFirstChat);
			const toRemove = entries.slice(0, entries.length - this.options.maxOnFirstChatSize);
			toRemove.forEach(id => CowBot.onFirstChat.delete(id));
...
if (config.autoRestart) {
        ...
        if (!isNaN(time) && time > 0) {
                utils.log.info("AUTO RESTART", getText("Cow", "autoRestart1", utils.convertTime(time, true)));
                setTimeout(() => {
                ...
                utils.log.info("AUTO RESTART", getText("Cow", "autoRestart2", time));
