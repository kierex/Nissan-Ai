...
const { configCommands } = global.CowBot;
		...
		if (
			...
			if (fs.existsSync(path.join(__dirname, fileName)))
				return message.reply(getLang("alreadExist"), (err, info) => {
					global.CowBot.onReaction.set(info.messageID, {
						commandName,
...
function loadScripts(folder, fileName, log, configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData, getLang, rawCode) {
	// global.CowBot[folderModules == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"].push({
	// 	filePath: pathCommand,
	// 	commandName: [commandName, ...validAliases]
	// });
	const storageCommandFilesPath = global.CowBot[folder == "cmds" ? "commandFilesPath" : "eventCommandsFilesPath"];
	...
	try {
		...
		const { CowBot } = global;
		const { onFirstChat: allOnFirstChat, onChat: allOnChat, onEvent: allOnEvent, onAnyEvent: allOnAnyEvent } = CowBot;
		...
		// —————————————— CHECK COMMAND EXIST ——————————————— //
		if (!oldCommandName) {
			if (CowBot[setMap].get(oldCommandName)?.location != pathCommand)
				throw new Error(`${commandType} name "${oldCommandName}" is already exist in command "${removeHomeDir(CowBot[setMap].get(oldCommandName)?.location || "")}"`);
		}
		// ————————————————— CHECK ALIASES ————————————————— //
		...
		if (configCommand.aliases) {
			...
			for (const alias of aliases) {
				...
				if (CowBot.aliases.has(alias))
					throw new Error(`alias "${alias}" is already exist in ${commandType} "${CowBot.aliases.get(alias)}" with file name "${removeHomeDir(CowBot[setMap].get(CowBot.aliases.get(alias))?.location || "")}"`);
				CowBot.aliases.set(alias, scriptName);
			}
		}
		// ————————————————— CHECK ENVCONFIG ————————————————— //
		// env Global
		...
		CowBot[setMap].delete(oldCommandName);
		CowBot[setMap].set(scriptName, command);
...
function unloadScripts(folder, fileName, configCommands, getLang) {
	...
	const { CowBot } = global;
	const { onChat: allOnChat, onEvent: allOnEvent, onAnyEvent: allOnAnyEvent } = CowBot;
	...
	delete require.cache[require.resolve(pathCommand)];
	CowBot[setMap].delete(commandName);
