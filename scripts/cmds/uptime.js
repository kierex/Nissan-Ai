module.exports = {
  ...
  run: async ({ api, event, db, config }) => {
    const start = new Date(db.getStat("start_time") || Date.now());
    const now = new Date();
    const ms = now - start;
    ...
    const regUsers = db.prepare("SELECT COUNT(*) as c FROM users WHERE registered = 1").get().c;
    ...
    api.sendMessage(
      ...
      `⚡ Commands Run: ${parseInt(totalCmds).toLocaleString()}\n` +
      `💾 Memory: ${memMB} MB\n` +
