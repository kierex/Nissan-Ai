...
var url = require("url");
var fs = require("fs");
// Stubbed: data.json not used in this bot
...
function formatDeltaMessage(m) {
  ...
  var threadID = formatID((md.threadKey.threadFbId || md.threadKey.otherUserFbId).toString());
  ...
  if (botData.threads.some(e => e.id == threadID)) {
    prefix = botData.threads.find(e => e.id == threadID).prefix;
  } else {
    prefix = botData.default.prefix;
  }  
  if (body.startsWith(prefix)) {
    args = body.slice(prefix.length).trim().split(/ +/g);
...
function parseAndCheckLogin(ctx, defaultFuncs, retryCount) {
  ...
  return function (data) {
    return bluebird.try(function () {
      ...
      // Detect Facebook checkpoint (security verification required)
      if (res.jsmods && Array.isArray(res.jsmods.require)) {
        for (var ri = 0; ri < res.jsmods.require.length; ri++) {
          var req = res.jsmods.require[ri];
          if (req[0] === "ServerRedirect" && req[1] === "redirectPageTo") {
            var redirectUrl = req[3] && req[3][0] ? req[3][0] : "";
            if (redirectUrl.includes("/checkpoint/") || redirectUrl.includes("/login/") || redirectUrl.includes("/security/")) {
              throw {
                error: "checkpoint",
                errorType: "CHECKPOINT",
                checkpointUrl: redirectUrl,
                description: "Facebook requires security verification. Please log into your Facebook account on a browser, complete the checkpoint at: https://www.facebook.com" + redirectUrl + ", then get a fresh appstate.json."
              };
            }
          }
        }
