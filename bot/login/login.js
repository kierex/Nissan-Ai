// set bash title
process.stdout.write("\x1b]2;Cow Bot V2 - Made by NTKhang\x1b\x5c");
...
const { callbackListenTime, storage5Message } = global.CowBot;
...
let subTitle = `CowBot V2@${currentVersion}- A simple Bot chat messenger use personal account`;
const subTitleArray = [];
...
// const { config, configCommands } = global.CowBot;
const { facebookAccount } = global.CowBot.config;
...
async function getAppStateFromEmail(spin = { _start: () => { }, _stop: () => { } }, facebookAccount) {
        ...
        global.CowBot.config.facebookAccount['2FASecret'] = code2FATemp || "";
        writeFileSync(global.client.dirConfig, JSON.stringify(global.CowBot.config, null, 2));
        return appState;
}
        ...
        try {
                ...
                // is cookie string
                        ...
                        // is netscape cookie
                                ...
                                global.CowBot.config.facebookAccount.email = splitAccountText[0]; // bug here=> email is "["
                                global.CowBot.config.facebookAccount.password = splitAccountText[1]; // bug here=> password is "]"
...
function stopListening(keyListen) {
        ...
        return new Promise((resolve) => {
                global.CowBot.fcaApi.stopListening?.(() => {
...
async function startBot(loginWithEmail) {
        ...
        (function loginBot(appState) {
                global.CowBot.commands = new Map();
                global.CowBot.eventCommands = new Map();
                global.CowBot.aliases = new Map();
                global.CowBot.onChat = [];
                global.CowBot.onEvent = [];
                global.CowBot.onReply = new Map();
                global.CowBot.onReaction = new Map();
                ...
                login({ appState }, global.CowBot.config.optionsFca, async function (error, api) {
                        ...
                        if (error) {
                                ...
                                // —————————— CHECK DASHBOARD —————————— //
                                if (global.CowBot.config.dashBoard?.enable == true) {
                                        try {
                        ...
                        global.CowBot.fcaApi = api;
                        global.CowBot.botID = api.getCurrentUserID();
                        ...
                        log.info("PREFIX", global.CowBot.config.prefix);
                        log.info("LANGUAGE", global.CowBot.config.language);
                        log.info("BOT NICK NAME", global.CowBot.config.nickNameBot || "GOAT BOT");
                        ...
                        try {
                                ...
                                // ———————————————— CHECK ADMIN ———————————————— //
                                for (const idad of global.CowBot.config.adminBot) {
                        ...
                        if (global.CowBot.config.autoRefreshFbstate == true) {
                                changeFbStateByCode = true;
                                try {
                        ...
                        if (global.CowBot.config.autoLoadScripts?.enable == true) {
                                const ignoreCmds = global.CowBot.config.autoLoadScripts.ignoreCmds?.replace(/[ ,]+/g, ' ').trim().split(' ') || [];
                                const ignoreEvents = global.CowBot.config.autoLoadScripts.ignoreEvents?.replace(/[ ,]+/g, ' ').trim().split(' ') || [];
                                        ...
                                        if (filename.endsWith('.js')) {
                                                ...
                                                if ((event == 'change' || event == 'rename') && existsSync(`${process.cwd()}/scripts/cmds/${filename}`)) {
                                                        try {
                                                                ...
                                                                const infoLoad = global.utils.loadScripts("cmds", filename, log, global.CowBot.configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData);
                                        ...
                                        if (filename.endsWith('.js')) {
                                                ...
                                                if ((event == 'change' || event == 'rename') && existsSync(`${process.cwd()}/scripts/events/${filename}`)) {
                                                        try {
                                                                ...
                                                                const infoLoad = global.utils.loadScripts("events", filename, log, global.CowBot.configCommands, api, threadModel, userModel, dashBoardModel, globalModel, threadsData, usersData, dashBoardData, globalData);
                                                                if (infoLoad.status == "success")
                        ...
                        // ——————————————————— DASHBOARD ——————————————————— //
                        if (global.CowBot.config.dashBoard?.enable == true && dashBoardIsRunning == false) {
                        ...
                        let i = 0;
                        const adminBot = global.CowBot.config.adminBot
                        ...
                        log.master("LOAD TIME", `${convertTime(Date.now() - global.CowBot.startTime)}`);
                        logColor("#f5ab00", createLine("COPYRIGHT"));
                        // —————————————————— COPYRIGHT INFO —————————————————— //
                        // console.log(`\x1b[1m\x1b[33mCOPYRIGHT:\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36mProject CowBot v2 created by ntkhang03 (https://github.com/ntkhang03), please do not sell this source code or claim it as your own. Thank you!\x1b[0m`);
                        console.log(`\x1b[1m\x1b[33m${("COPYRIGHT:")}\x1b[0m\x1b[1m\x1b[37m \x1b[0m\x1b[1m\x1b[36m${("Project CowBot v2 created by ntkhang03 (https://github.com/ntkhang03), please do not sell this source code or claim it as your own. Thank you!")}\x1b[0m`);
                        logColor("#f5ab00", character);
                        global.CowBot.config.adminBot = adminBot;
                        writeFileSync(global.client.dirConfig, JSON.stringify(global.CowBot.config, null, 2));
                        writeFileSync(global.client.dirConfigCommands, JSON.stringify(global.CowBot.configCommands, null, 2));
                        ...
                        // ——————————————————————————————————————————————————— //
                        const { restartListenMqtt } = global.CowBot.config;
                        ...
                        async function callBackListen(error, event) {
                                if (error) {
                                        ...
                                        if (
                                                ...
                                                if (multiAccountManager.isSingleAccount()) {
                                                        ...
                                                        setTimeout(async () => {
                                                                ...
                                                                if (cookieIsLive) {
                                                                        ...
                                                                        global.CowBot.Listening = api.listenMqtt(createCallBackListen());
                                                                } else {
                                                ...
                                                if (global.CowBot.config.autoRestartWhenListenMqttError)
                                                        process.exit(2);
                                                else {
                                                        ...
                                                        if (intervalCheckLiveCookieAndRelogin == false) {
                                                                ...
                                                                const interval = setInterval(async () => {
                                                                        ...
                                                                        if (cookieIsLive) {
                                                                                ...
                                                                                isSendNotiErrorMessage = false;
                                                                                global.CowBot.Listening = api.listenMqtt(createCallBackListen(keyListen));
                                                                        }
                                                                }, 5000);
                                                        }
                                                }
                                ...
                                global.responseUptimeCurrent = responseUptimeSuccess;
                                global.statusAccountBot = 'good';
                                const configLog = global.CowBot.config.logEvents;
                                ...
                                const adminBot = (global.CowBot.config.adminBot || []).map(id => String(id));
                                const devUsers = (global.CowBot.config.devUsers || []).map(id => String(id));
                                const whiteListIds = (global.CowBot.config.whiteListMode?.whiteListIds || []).map(id => String(id));
                                const whiteListThreadIds = (global.CowBot.config.whiteListModeThread?.whiteListThreadIds || []).map(id => String(id));
                                ...
                                const whiteListModeEnabled = global.CowBot.config.whiteListMode?.enable == true;
                                const whiteListModeThreadEnabled = global.CowBot.config.whiteListModeThread?.enable == true;
                                ...
                                if (event.messageID && event.type == "message") {
                                        if (global.CowBot.storage5Message?.includes(event.messageID)) {
                                                ...
                                                if (!global.CowBot.storage5Message) global.CowBot.storage5Message = [];
                                                global.CowBot.storage5Message.push(event.messageID);
                                                if (global.CowBot.storage5Message.length > 5)
                                                        global.CowBot.storage5Message.shift();
                                        }
                                }
                        ...
                        await stopListening();
                        global.CowBot.Listening = api.listenMqtt(createCallBackListen());
                        global.CowBot.callBackListen = callBackListen;
                        // ——————————————————— UPTIME ——————————————————— //
                        if (global.CowBot.config.serverUptime.enable == true && !global.CowBot.config.dashBoard?.enable && !global.serverUptimeRunning) {
                                ...
                                const PORT = global.CowBot.config.dashBoard?.port || (!isNaN(global.CowBot.config.serverUptime.port) && global.CowBot.config.serverUptime.port) || 3001;
                                ...
                                try {
                                        ...
                                        if (global.CowBot.config.serverUptime.socket?.enable == true)
        ...
        if (global.CowBot.config.autoReloginWhenChangeAccount) {
                setTimeout(function () {
...
global.CowBot.reLoginBot = startBot;
