		...
		for (const data of aliasesData) {
			...
			for (const alias of aliases)
				if (CowBot.aliases.has(alias))
					...
					CowBot.aliases.set(alias, commandName);
		}
	}
	...
	for (const folderModules of folders) {
		...
		for (const file of Files) {
			...
			try {
				...
				if (CowBot[setMap].has(commandName))
					throw new Error(`${text} "${commandName}" already exists with file "${removeHomeDir(CowBot[setMap].get(commandName).location || "")}"`);
				...
				if (aliases) {
					...
					for (const alias of aliases) {
						...
						if (CowBot.aliases.has(alias))
							throw new Error(`alias "${alias}" already exists in ${text} "${CowBot.aliases.get(alias)}" with file "${removeHomeDir(CowBot[setMap].get(CowBot.aliases.get(alias))?.location || "")}"`);
				...
				// ——————————————— CHECK RUN ANYTIME ——————————————— //
				if (onChat)
					CowBot.onChat.push(commandName);
				// ——————————————— CHECK ONFIRSTCHAT ——————————————— //
				if (onFirstChat)
					CowBot.onFirstChat.push({ commandName, threadIDsChattedFirstTime: [] });
				// ————————————————— CHECK ONEVENT ————————————————— //
				if (onEvent)
					CowBot.onEvent.push(commandName);
				// ———————————————— CHECK ONANYEVENT ———————————————— //
				if (onAnyEvent)
					CowBot.onAnyEvent.push(commandName);
				// —————————————— IMPORT TO GLOBALGOAT —————————————— //
				CowBot[setMap].set(commandName.toLowerCase(), command);
				commandLoadSuccess++;
