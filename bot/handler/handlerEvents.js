...
async function trackCommandSpam(threadID, threadName, globalData, message) {
	const config = global.CowBot.config;
...
module.exports = function (api, threadModel, userModel, dashBoardModel, globalModel, usersData, threadsData, dashBoardData, globalData) {
        return async function (event, message) {
                ...
                const { utils, client, CowBot } = global;
                const { getPrefix, removeHomeDir, log, getTime } = utils;
                const { config, configCommands: { envGlobal, envCommands, envEvents } } = CowBot;
                ...
                */
                ...
                async function onStart() {
                        ...
                        let command = CowBot.commands.get(commandName) || CowBot.commands.get(CowBot.aliases.get(commandName));
                        // ———————— CHECK ALIASES SET BY GROUP ———————— //
                        const aliasesData = threadData.data.aliases || {};
                        for (const cmdName in aliasesData) {
                                if (aliasesData[cmdName].includes(commandName)) {
                                        command = CowBot.commands.get(cmdName);
                                        break;
                                }
                        }
                        // ————————————— SET COMMAND NAME ————————————— //
                        ...
                        if (!command) {
                                ...
                                if (!hideNotiMessage.commandNotFound) {
                                        ...
                                        function getCachedCommandNames() {
                                                const cmdCount = CowBot.commands.size;
                                                const aliasCount = CowBot.aliases.size;
                                                const cache = CowBot._cmdNameCache || {};
                                                if (!cache.list || cache.cmdCount !== cmdCount || cache.aliasCount !== aliasCount) {
                                                        const list = [...CowBot.commands.keys(), ...CowBot.aliases.keys()];
                                                        CowBot._cmdNameCache = {
                                                                list,
                ...
                */
                async function onChat() {
                        const allOnChat = CowBot.onChat || [];
                        const args = body ? body.split(/ +/) : [];
                        for (const key of allOnChat) {
                                const command = CowBot.commands.get(key);
                                if (!command)
                                        continue;
                                const commandName = command.config.name;
                ...
                */
                async function onAnyEvent() {
                        const allOnAnyEvent = CowBot.onAnyEvent || [];
                        let args = [];
                ...
                */
                async function onFirstChat() {
					...
					// Commands register themselves in CowBot.onChat with a flag for firstChat
					if (CowBot.onFirstChat.has(threadID))
						return;

					const args = body ? body.split(/ +/) : [];

					for (const commandName of CowBot.onFirstChat._commandNames || []) {
						const command = CowBot.commands.get(commandName);
					...
					CowBot.onFirstChat.add(threadID);
                }
                ...
                */
                async function handlerEvent() {
                        const { author } = event;
                        const allEventCommand = CowBot.eventCommands.entries();
                        for (const [key] of allEventCommand) {
                                const getEvent = CowBot.eventCommands.get(key);
                                if (!getEvent)
                                        continue;
                ...
                */
                async function onEvent() {
                        const allOnEvent = CowBot.onEvent || [];
                        const args = [];
                        const { author } = event;
